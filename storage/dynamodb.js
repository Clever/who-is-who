...
function checkSchema(expected, actual) {
	...
	if(_.isEqual(expected, minactual)) { return null; }
	else {
		return new Error(
			"Mismatched table schema for " + expected.TableName + "\n" +
...
function createTableIfNeeded(dynamodb, table, cb) {
	dynamodb.describeTable({ TableName: table.TableName }, (err, data) => {
		if(!err) { return cb(checkSchema(table, data.Table)); }
...
function createTablesIfNeeded(endpoint, region, accessId, secretKey, cb) {
	let dynamodb = new aws.DynamoDB({
		endpoint: endpoint, region: region, accessKeyId: accessId, secretAccessKey: secretKey
	});
...
function createWorkingExport(endpoint, region, accessId, secretKey) {
	...
	let toObj = (obj, cb) => {
		...
		client.get(params, (err, data) => { cb(err, data.Item); });
	};
...
module.exports = function(endpoint, region, accessId, secretKey, tableNameSuffix) {
	...
	createTablesIfNeeded(endpoint, region, accessId, secretKey, (err) => {
		...
		Object.keys(working).forEach((key) => { loading[key] = working[key]; });
	...
	let loading = {
		all(cb) { waitForCreate(() => loading.all(cb)); },
		has(key, cb) {  waitForCreate(() => loading.has(key, cb)); },
		by(key, val, cb) {  waitForCreate(() => loading.by(key, val, cb)); },
		history(whoid, path, cb) {  waitForCreate(() => loading.history(whoid, path, cb)); },
		put(cur, diffs, cb) {  waitForCreate(() => loading.put(cur, diffs, cb)); },
	};

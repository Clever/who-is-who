...
function toPaths(obj) {
	...
	while((node = nodes.pop())) {
		...
		if(!_.isPlainObject(obj)) { paths[path] = obj; continue; }
		else { paths[path] = {}; } // Empty object means path contains a sub object
...
function generateDiffs(author, prev, cur) {
	...
	Object.keys(curPaths).forEach((path) => {
		...
		if(path in prevPaths) { diffs[path] = { prev: prev, cur: cur }; }
		else { diffs[path] = { created: true, cur: cur }; }
...
module.exports = function(storage) {
	return {
		all(cb) { storage.all(cb); },
		has(key, cb) { storage.has(key, cb); },
		by(key, val, cb) {
		...
		one(key, val, cb) {
			...
			this.by(key, val, (err, results) => {
				...
				if(results.length !== 1) { return cb(new Error("More than one value found.")); }
		...
		history(key, val, path, cb) {
			this.one(key, val, (err, obj) => {
				if(err) { return cb(err); }
				if(obj == null) { return cb(null, null); }
		...
		put(author, key, val, body, cb) {
			...
			this.one(key, val, (err, obj) => {
				...
				if(!cur.email) { return cb(UserError("Can't save.  No email address found.")); }

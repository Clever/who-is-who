...
function omitId(objs) {
	if(Array.isArray(objs)) { return objs.map(omitId); }
	else if(_.isPlainObject(objs)) { return _.mapValues(_.omit(objs, "_whoid"), omitId); }
	else { return objs; }
}
...
function genericResponder(res) {
	return (err, data) => {
		if(err) {
			if(err.isUserError) { return res.status(400).json({ "error": err.toString() }); }
			
			return res.status(500).json({ "error": err.toString() });	
		}
		if(data == null) { return res.status(404).json({ "error": "not-found" }); }
		if(data != null && _.isEmpty(data)) { return res.status(404).json(data); }
	...
	return bee.route({
		...
		"/alias/`key`/`value`": {
			...
			"POST PUT": (req, res) => {
				...
				if(!author) { return res.status(400).json({ "error": "no X-WIW-Author header" }); }
		...
		"/list/`key`/`value`/data/`path...`": {
			"GET": (req, res) => {
				...
				db.by(key, value, (err, data) => {
					if(err) { return res.status(500).json({ "error": err.toString() }); }
					if(_.isEmpty(data)) { return res.status(404).json(data); }
